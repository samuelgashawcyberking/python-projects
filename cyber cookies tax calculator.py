...
def btn3f():
    ...
    if name and phone and money and position and tax:
        ...
        T.save(file2_path)
    if name and phone and money and position:
        messagebox.showinfo('status', 'data saved')
    else:
        messagebox.showwarning('warning', 'please fill all filds to save')


def btn1f():
    money = money_entry.get()
    if money:
        messagebox.showinfo('status','monthly amount is enterd ')

    else:
        messagebox.showwarning('status','please enter your money per month')
